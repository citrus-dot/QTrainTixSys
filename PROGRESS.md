...
| 项目 | 说明 |
|------|------|
| 当前版本 | V16（前端全面优化，待推送 dev） |
| 技术栈 | Qt 6.5+ Widgets / C++17 / CMake |
| 代码结构 | `Seat`/`Train`/`TrainSystem` 数据层 + `MainWindow` + 组件（`SidebarWidget`/`TrainListView`/`SeatTableView`/`QueryPage`）+ 对话框（新增/售票/车票）+ `FadeDialog` 基类 + `theme.qss` 主题 |
| 开发分支 | 新仓库 `summer2026class1-213251605` 的 `dev` 分支 |
| 现有功能 | 班次增删、售票/退票、余票与可选座位查询、中间停靠、数据文件读写；V7 含日期/票价/座位等级与输入校验；V8 售票/退票座位可视化网格；V9 售票后车票展示与导出；V10 班次筛选与搜索；审查精简：座位登记表仅显示已售、查询可选座位按车厢显示；V11 数据层单元测试（Qt Test + CTest）；V12 组件化重构（`TrainListView`/`SeatTableView`），MainWindow 精简；V16 前端全面优化（浅色扁平主题、左侧导航栏、顶部操作栏、查询页、淡入动效、空状态引导、SVG 图标） |
...
| 10 | V16 | 前端全面优化（UI/交互） | 易用性/美观 | 迭代 6 |

> 扩展原则（2026-09-02）：测试先行（重构前建安全网）；一个功能对应一次提交，每次提交独立可验证；功能求实用，代码求简单易懂。
> V16 说明（2026-09-03）：纯 UI/交互优化，不引入外部依赖；V13–V15 的功能项（快捷操作/修改班次/未保存提示）暂搁置，界面联动必需时再补。
...
### 迭代 10（V16）：前端全面优化（UI/交互）—— 已完成，待推送 dev

**目标**：全面优化界面易用性与美观度，达到生产级软件标准。功能入口明显易用、操作简便流畅、界面干净有设计感、交互反馈自然舒适。

**涉及文件**：
- 新增：`sidebarwidget.h/cpp`（左侧导航栏）、`fadedialog.h/cpp`（淡入对话框基类）、`querypage.h/cpp/ui`（查询页）、`theme.qss`（全局主题）、`resources.qrc`（资源清单）、`icons/*.svg`（9 个线性图标）
- 修改：`mainwindow.h/cpp/ui`、`trainlistview.h/cpp`、`seattableview.h/cpp`、`ticketdialog.h/cpp/ui`、`addtraindialog.h/cpp/ui`、`ticketview.h/cpp/ui`、`main.cpp`（加载主题）、`CMakeLists.txt`（集成新组件与资源）
- 删除：`querydialog.h/cpp/ui`（查询对话框整合为查询页）

**实现细节**：

1. **浅色现代扁平主题（`theme.qss`）**：品牌蓝 `#2F6FED` + 中性灰 + 圆角（8/12px）卡片式设计，统一按钮/输入框/表格/菜单/滚动条/提示等全部控件风格；主操作按钮（售票）蓝色强调、危险操作（删除）红色强调。
2. **左侧导航栏（`SidebarWidget`）**：品牌区 + 「班次管理」「查询」导航 + 底部「关于」；`QButtonGroup` 管理选中态，选中项高亮；固定 188px 宽。
3. **顶部操作栏**：打开/保存 + 新增/删除/售票/退票 全部按钮化，配 SVG 图标，售票为主操作蓝色强调；核心功能不再藏在菜单栏。
4. **页面切换**：`QStackedWidget` 双页面（班次管理/查询），侧边栏切换并同步顶部标题。
5. **查询整合**：原模态查询对话框改为独立 `QueryPage` 页面嵌入主窗口，随数据刷新，无需弹窗。
6. **淡入动效（`FadeDialog`）**：所有对话框（新增/售票/车票）显示时 160ms 窗口淡入，反馈自然。
7. **空状态引导**：`TrainListView`/`SeatTableView` 无数据时显示引导文案（如"点击上方「新增班次」或先「打开」数据文件"）。
8. **座位可视化**：`TicketDialog` 圆角座位按钮网格，三态区分（空座绿/已售红/已选蓝），售票/退票分段切换。
9. **SVG 图标**：统一线性图标风格，经 `resources.qrc` 集成，跨平台清晰显示。
10. **状态栏统计**：班次总数 / 余票总数 / 已售总数常驻显示，数据变化时同步刷新。

**决策点**：
- **D17 视觉风格**：A. 浅色现代扁平（建议）；B. 深色主题。
- **D18 功能入口**：A. 左侧导航栏 + 顶部操作栏（建议）；B. 仅工具栏。
- **D19 动效程度**：A. 克制自然（对话框淡入，建议）；B. 丰富动效（页面滑动等）。
- **D20 查询入口**：A. 独立页面（建议）；B. 保留模态对话框。

**验收**：功能入口明显易用（核心操作一键可达）；操作简便流畅（空状态引导、双击售票）；界面干净美观有设计感；交互反馈自然（淡入/悬停/按下状态）；ctest 全部通过。
...
| D17 | 视觉风格 | 浅色现代扁平 / 深色 | 浅色现代扁平 |
| D18 | 功能入口 | 左侧导航栏 + 顶部操作栏 / 仅工具栏 | 左侧导航栏 + 顶部操作栏 |
| D19 | 动效程度 | 克制自然（淡入）/ 丰富动效 | 克制自然（对话框淡入） |
| D20 | 查询入口 | 独立页面 / 模态对话框 | 独立页面 |
