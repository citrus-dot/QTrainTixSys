...
| 11 | V17 | 统计可视化 + 仿真车票 | UI 亮点 | 迭代 6 |
| 12 | V18 | V13–V15 功能补齐 | 功能/工程 | 迭代 5、6 |
...
> V18 说明（2026-09-06）：V13–V15 搁置项全部补齐——其中工具栏按钮/双击售票/状态栏统计已被 V16 覆盖，本次补齐 列头排序、身份证脱敏统一、修改班次（V14）、未保存提示+窗口记忆（V15）。
...
### 迭代 7（V13）：快捷操作与统计 —— 已完成（工具栏/双击/统计由 V16 覆盖，其余 V18 补齐）
...
### 迭代 8（V14）：修改班次 —— 已完成（V18）

**V18 实现说明（2026-09-06）**：
- `TrainSystem::updateTrain(oldNo, newTrain)`：按 oldNo 定位替换；新班次号查重（排除自身）；已售座位超出新编组范围时拒绝；成功时迁移原售座记录。`indexOf()` 私有辅助复用于 add/update/remove。
- `AddTrainDialog` 编辑模式构造（传入 `Train` 回填字段，标题"修改班次"）。
- 「一等车厢数」Spin 升级为「一等车厢号」文本输入（逗号分隔，如 `1` 或 `2,3`）：原前缀式布局无法表达一等厢在中/后部的班次（如 D2206、G1371），编辑会造成等级布局丢失。
- 主窗口顶部操作栏新增「修改班次」按钮（选中班次后可用，图标 `action-edit.svg`）。
- 测试：tst_system 新增 4 例（改号成功/改号冲突/缩编组拦截×2/成功保留售座）。
...
### 迭代 9（V15）：未保存提示 + 窗口记忆 —— 已完成（V18）

**V18 实现说明（2026-09-06）**：
- `MainWindow::m_dirty` 脏标记：新增/修改/删除班次及售票/退票/座位登记的 `dataChanged` 处置位；新建/打开/保存后清除。
- `closeEvent()`：脏状态下弹 保存/不保存/取消 三选；选"保存"但保存被取消或失败时中止关闭。
- `QSettings("SEU", "TrainSystem")`：关闭时写窗口几何与上次文件路径；启动时 `restoreWindowState()` 恢复几何并自动打开上次文件（存在时）。
...
### V16 修正（2026-09-03，前端细节打磨）

**目标**：修复 V16 落地后暴露的布局/动效/图标/交互问题，提升自然度。

**改动**：

1. **修复切换页面白屏 bug**：移除 `QGraphicsOpacityEffect` 页面切换淡入与启动淡入（该机制在 macOS 上导致页面透明度滞留为 0、整屏变白、悬停才显示元素）。保留 `FadeDialog` 的 `windowOpacity` 对话框淡入（独立窗口，无此问题）。
2. **空状态引导移入表格框内**：`TrainListView`/`SeatTableView` 用 `QStackedLayout` 在表格框内切换空状态（含火车图标 + 引导文案）；修复初始启动未触发空状态的问题（构造函数补 `refreshTrainList()`）。
3. **班次列表全宽 + 座位登记弹窗**：班次管理页移除右侧 `SeatTableView` 分栏，班次列表占满全宽；座位登记改为 `SeatTableDialog` 弹窗（`FadeDialog` 基类）。
4. **侧边栏加宽填充**：`SidebarWidget` 加宽至 220px，品牌区 + 副标题 + 「功能导航」分组 + 信息卡 + 版本号，弥补上下空旷；导航按钮文字增大。
5. **下拉箭头改 V 形**：`QComboBox::down-arrow` 改用 V 形 PNG 图标（`chevron-down.png`）。
6. **查询页信息卡美化**：`QueryPage` 改为字段化网格布局（日期/发车时间/发车城市/终点城市/余票/票价/等级），加分隔线。
7. **选中交互增强**：查询页可选座位号点击 → `SeatMapPopup` 座位周边分布浮窗；中间停靠站点击 → `QToolTip` 提示。
8. **按钮颜色统一**：移除售票按钮蓝色主样式，所有按钮默认白色；悬停/按下变品牌蓝，图标同步切换为白色变体（`*-white.svg`），保证蓝底白图清晰。
9. **图标替换**：退票图标改为返回箭头（更贴合"退票/退回"语义）；座位登记图标改为铅笔样式（简约线条风）。
10. **按钮状态联动**：无选中班次时禁用 删除/售票/退票/座位登记（`onTrainSelectionChanged`），引导式交互。

**决策点**：
- **D21 页面切换动效**：A. 移除淡入（建议，规避白屏 bug，切换即时）；B. 换用无 `QGraphicsOpacityEffect` 的轻量动效。
- **D22 座位登记形式**：A. 弹窗（建议，班次列表全宽）；B. 附在下方分栏。

**验收**：切换页面不再白屏；空状态在表格框内显示；班次列表全宽可用；侧边栏饱满；下拉箭头为 V 形；按钮默认白、悬停蓝、图标变白；退票/座位登记图标贴合语义；ctest 全部通过。

---

### 迭代 12（V18）：V13–V15 功能补齐 —— 已完成（2026-09-06）

**目标**：补齐 V16 时期搁置的 V13–V15 功能项，版本号升至 v0.3.0。

**实现细节**：

1. **班次列表列头排序**（V13.3）：`TrainListView` 开启 `setSortingEnabled`，填充数据期间临时关闭防插入错乱；车厢数/每厢座位数/余票三列使用 `NumericItem`（按数值比较，避免 "99" 排在 "100" 后）。
2. **身份证脱敏统一**（V13.5）：`TicketCard::maskId` 下沉为 `tableutil.h` 的 `maskId()` 共享辅助；票面、车票导出 txt 原本已脱敏，本次补齐座位登记浮窗 tooltip；数据文件仍存完整号。
3. **修改班次**（V14）：详见迭代 8 的 V18 实现说明。
4. **未保存提示 + 窗口记忆**（V15）：详见迭代 9 的 V18 实现说明。
5. **测试数据随机化**：各班次上座率加偏移（29.3%/38.2%/31.3%/30.0%/52.5%），总体 33.8%，避免全为整数占比。
6. **版本号**：CMake bundle、关于页、侧边栏统一升至 v0.3.0。

**验收**：构建零警告；ctest 2/2 通过（含 updateTrain 4 例新断言）；冒烟渲染确认工具栏新按钮与排序后表格正常。
...
| D21 | 页面切换动效 | 移除淡入 / 轻量动效 | 移除淡入（规避白屏 bug） |
| D22 | 座位登记形式 | 弹窗 / 下方分栏 | 弹窗（班次列表全宽） |
